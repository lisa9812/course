...
$LM:M/P=(1/V)P$（M是名义货币供应，P是价格水平，Y是实际收入/支出，V是货币周转速度，$1/V$是人们愿意为1单位$Y$持有的货币数量）
### IS曲线
#### 关于C=C(y)
性质:        
...
nominal intersts = real intersts + expected inflation  

Fisher effect：real interst are stable. changes in interest rates are driven by changes in expected inflation.


实际上，nominal intersts = real intersts + expected inflation + riskpremium for uncertainty
...
常用的操作方式是 *repurchase agreement*，The central bank purchases securities from banks that, in turn, agree to repurchase the securities at a higher price in the future.
